...
class TestAPIRoles:
    ...
    def test02_api_check_get_roles(self, api_tester, auth_token):
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Roles result: {result}")
        else:
            logger.info(f"Roles check response: {response.status_code} - {response.text}")
    ...
    def test03_api_check_post_roles(self, api_tester, auth_token):
        ...
        if response.status_code == 201:
            ...
            logger.info(f"Role created successfully with ID: {api_tester.created_role_id}")
        else:
            logger.info(f"Role creation failed: {response.status_code} - {response.text}")
    ...
    def test04_api_check_patch_roles(self, api_tester, auth_token):
        ...
        logger.info(f"Attempting to update role with ID: {api_tester.created_role_id}")
        ...
        logger.info(f"Update role response status: {response.status_code}")
        logger.info(f"Update role response content: {response.text}")
        logger.info(f"Request URL: {response.url}")
        logger.info(f"Request cookies sent: {response.request.headers.get('Cookie', 'No cookies')}")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Role update response: {response.status_code} - {response.text}")
    ...
    def test06_api_check_post_new_policy_roles(self, api_tester, auth_token):
        ...
        logger.info(f"Attempting to add policy to role with ID: {api_tester.created_role_id}")
        logger.info(f"Using policy ID: {params['policy_id']}")
        response = api_tester.session.post(
        ...
        logger.info(f"Add policy to role response content: {response.text}")
        logger.info(f"Request URL: {response.url}")
        logger.info(f"Request cookies sent: {response.request.headers.get('Cookie', 'No cookies')}")
        
        # L'API peut retourner 200 OK ou 201 Created si les paramètres sont corrects
        assert response.status_code in [200, 201, 400], f"Unexpected status: {response.status_code} - {response.text}"
        assert response.status_code in [200, 201], f"Expected 200 OK or 201 Created for adding policy to role, got {response.status_code}: {response.text}"
        
        if response.status_code in [200, 201]:
            result = response.json()
            assert "id" in result, "Expected relation ID in response"
            logger.info(f"Policy added to role successfully: {result}")
            # Stocker le policy_id pour le test de suppression
            api_tester.created_policy_id = result["policy_id"]
        else:
            logger.info(f"Add policy to role response: {response.status_code} - {response.text}")
    ...
    def test07_api_check_get_policy_roles(self, api_tester, auth_token):
        ...
        logger.info(f"Get policies of role response status: {response.status_code}")
        logger.info(f"Get policies of role response content: {response.text}")
        logger.info(f"Request URL: {response.url}")
        logger.info(f"Request cookies sent: {response.request.headers.get('Cookie', 'No cookies')}")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Policies of role retrieved successfully: {result}")
        else:
            logger.info(f"Get policies of role response: {response.status_code} - {response.text}")
    ...
    def test08_api_check_delete_policy_roles(self, api_tester, auth_token):
        ...
        logger.info(f"Attempting to delete policy with ID: {policy_to_delete_id} from role ID: {api_tester.created_role_id}")
        ...
        logger.info(f"Delete policy from role response content: {response.text}")
        logger.info(f"Request URL: {response.url}")
        logger.info(f"Request cookies sent: {response.request.headers.get('Cookie', 'No cookies')}")
        ...
        if response.status_code == 204:
            logger.info(f"Policy deleted from role successfully: ID {policy_to_delete_id}")
        else:
            logger.info(f"Delete policy from role response: {response.status_code} - {response.text}")
    ...
    def test09_api_check_delete_roles(self, api_tester, auth_token):
        ...
        logger.info(f"Attempting to delete role with ID: {api_tester.created_role_id}")
        ...
        logger.info(f"Delete role response content: {response.text}")
        logger.info(f"Request URL: {response.url}")
        logger.info(f"Request cookies sent: {response.request.headers.get('Cookie', 'No cookies')}")
        ...
        if response.status_code == 204:
            logger.info(f"Role deleted successfully: ID {api_tester.created_role_id}")
            del api_tester.created_role_id  # Nettoyer l'ID du rôle supprimé
        elif response.status_code == 404:
            logger.info(f"Role not found (already deleted?): ID {api_tester.created_role_id}")
            # Nettoyer l'ID même si le rôle n'existe plus
            if hasattr(api_tester, 'created_role_id'):
                del api_tester.created_role_id
        else:
            logger.info(f"Role deletion response: {response.status_code} - {response.text}")